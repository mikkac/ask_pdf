...
class RAGChat:
    ...
    Attributes:
        max_history_tokens (int): Maximum number of tokens to retain in conversation history.
        conversation_history (list): List to store the history of the conversation.
    ...
    Methods:
        ...
        _update_history(message): Updates the conversation history with the given message.
        _build_context(): Builds the conversational context from the history.
    """
    ...
    def __init__(self, max_history_tokens, openai_api_key):
        ...
        Args:
            max_history_tokens (int): The maximum number of tokens to retain in the conversation history.
    ...
    def send_message(self, user_msg):
        ...
        """
        self._update_history(f"User: {user_msg}")

        context_combined = self._build_context()
        prompt = f"Here is the context of our conversation: {context_combined}. And here is the new message: User: {user_msg}"
        response = self.automerging_query_engine.query(prompt)
        self._update_history(f"AI: {response}")
    ...
    def _update_history(self, message):
        """
        Updates the conversation history with the given message.

        This method ensures that the total number of tokens in the history does not exceed the set limit.

        Args:
            message (str): The message (user's or AI's) to add to the conversation history.
        """
        self.conversation_history.append(message)
        # Remove oldest messages if token count exceeds limit
        while (
            len(" ".join(self.conversation_history).split()) > self.max_history_tokens
        ):
            self.conversation_history.pop(0)

    def _build_context(self):
        """
        Builds the conversational context from the stored history.

        This method concatenates the messages in the conversation history to form the context.

        Returns:
            str: The concatenated string of messages forming the conversation context.
        """
